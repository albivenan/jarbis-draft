...
export default function PeraturanSOP({ sops }: { sops: SOP[] }) {
  ...
  const handleDelete = (sop: SOP) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus SOP "${sop.title}"?`)) {
      router.delete(route('hrd.administrasi.sop.destroy', sop.id));
    }
  };
  ...
  return (
    <AuthenticatedLayout
      ...
      breadcrumbs={[
        ...
        { title: 'Peraturan & SOP', href: '/roles/hrd/administrasi/sop' }
      ]}
    >
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            <Link href={route('hrd.administrasi.sop.create')}>
              <Button>
              ...
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex flex-col gap-2 ml-6">
                    <Link href={route('hrd.administrasi.sop.show', sop.id)}>
                    ...
                    {sop.attachments && sop.attachments.length > 0 && (
                      <Button size="sm" variant="outline" onClick={() => alert(`Download: ${sop.attachments[0]}`)}>
                        <Download className="h-4 w-4 mr-2" />
                        Download
                      </Button>
                    )}
                    <Link href={route('hrd.administrasi.sop.edit', sop.id)}>
                      <Button size="sm" variant="outline">
                        <Edit className="h-4 w-4 mr-2" />
                        Edit
                      </Button>
                    </Link>
                    <Button size="sm" variant="destructive" onClick={() => handleDelete(sop)}>
