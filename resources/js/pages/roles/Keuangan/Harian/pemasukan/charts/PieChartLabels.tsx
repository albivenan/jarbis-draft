...
export const RenderCustomizedLabelWithLine = (props: RenderCustomizedLabelWithLineProps) => {
    ...
    const currentLabelData = labelsInfo[index];
    if (!currentLabelData) return null;
    ...
    const sideGroup = labelsInfo.filter((label: any) => {
        ...
        if (isRightSide) {
            if (a.midAngle > 90) a.midAngle -= 360;
    ...
    const color = JENIS_COLORS[jenisPemasukanValues[index]];
    ...
    <ul className="recharts-default-legend" style={{ padding: 0, margin: 0 }}>
      {(payload ?? [])
        ...
        .map((entry: any, index: number) => {
          const fullJenisName = entry.value;
          const itemColor =
            (fullJenisName && JENIS_COLORS[fullJenisName as JenisPemasukan]) ||
            entry.color;

          if (!fullJenisName) return null;
