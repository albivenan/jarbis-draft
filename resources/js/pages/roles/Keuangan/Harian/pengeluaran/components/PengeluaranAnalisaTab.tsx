...
import { JENIS_PENGELUARAN, jenisPengeluaranValues, JENIS_COLORS, JenisPengeluaran } from '../utils/constants';
import { formatCurrency } from '../utils/formatters';
import { RenderCustomizedLabelWithLine, CustomPieTooltip, CustomPieChartLegend } from '../charts/PieChartLabels';
import { formatCompact } from '../utils/formatters';
import { CustomizedLabel, CustomTooltip } from '../charts/ComposedChartLabels';
import { startOfDay } from 'date-fns';

interface PengeluaranAnalisaTabProps {
...
const PengeluaranAnalisaTab: React.FC<PengeluaranAnalisaTabProps> = ({
    analysisData,
    isFilterModalOpen,
    setFilterModalOpen,
    dateFilter,
            ...
            <Card>
                <CardHeader>
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                        <div>
                            <CardTitle>Ringkasan & Komposisi Pengeluaran</CardTitle>
                        ...
                        <div className="flex items-center gap-2 mt-4 sm:mt-0 flex-wrap">
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent>
                                    ...
                                    {jenisPengeluaranValues.map((jenis: JenisPengeluaran) => (
                ...
                <CardContent className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center pt-6">
                    <div>
                        <div className="space-y-4">
                            <div>
                                <p className="text-sm text-muted-foreground">Total Pengeluaran (dari Jenis Terpilih)</p>
                            ...
                            <div className="space-y-2">
                                {analysisJenisFilter.length > 0 ? (
                                    ...
                                    <p className="text-sm text-muted-foreground">Tidak ada jenis pengeluaran yang dipilih.</p>
                                )}
                            </div>
                        </div>
                    </div>
                    <div className="relative min-h-[280px] h-[320px] w-full min-w-0">
                        {analysisData.pieChartData.length === 0 || analysisData.pieChartData.every(entry => entry.value === 0) ? (
                            <div className="flex flex-col items-center justify-center h-full text-center">
                                ...
                                <p className="mt-1 text-sm text-gray-400">Tidak ada data pengeluaran untuk periode atau filter yang dipilih.</p>
                            </div>
                        ) : (
                                ...
                                <PieChart>
                                    ...
                                    >
                                        {analysisData.pieChartData.map((entry: { name: string; value: number }, index: number) => (
                                            <Cell key={`cell-${index}`} fill={JENIS_COLORS[jenisPengeluaranValues[index]]} />
                                        ))}
                                    </Pie>
            ...
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-center">
                        <div>
                            <CardTitle>Tren Pengeluaran per Periode</CardTitle>
                            <CardDescription>Grafik menunjukkan tren pengeluaran berdasarkan jenis yang dipilih dalam periode waktu.</CardDescription>
                        </div>
                        <Button
                            variant="outline"
                            size="sm"
                                    ...
                                    .map((key: any) => {
                                        if (key.type === 'area') {
                                            if (analysisJenisFilter.includes(key.dataKey)) {
                                                return <Area key={key.dataKey} {...key} connectNulls fillOpacity={0.5} />;
                                            }
                                        }
                                        return null;
                                    })}
...
export default PengeluaranAnalisaTab;
