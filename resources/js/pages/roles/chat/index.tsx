...
export default function ChatSupervisorsPage() {
  ...
  return (
    <AuthenticatedLayout>
      <Head title="Chat Atasan" />
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold">Chat Atasan</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-lg rounded-lg bg-background p-4 shadow">
            <div className="mb-2 flex items-center justify-between">
              <h2 className="text-lg font-semibold">Bantuan • Chat Atasan</h2>
              <button aria-label="Tutup" onClick={() => setShowHelp(false)} className="rounded p-1 hover:bg-accent">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>Pilih kanal (Umum/Finance/Produksi/HRD) untuk berdiskusi lintas atasan. Ketik pesan dan tekan Enter untuk mengirim.</p>
              <ul className="list-disc pl-5 space-y-1">
                <li>Daftar peserta terlihat di sisi kanan.</li>
                <li>Riwayat percakapan disimpan per kanal (mock data, integrasi backend selanjutnya).</li>
              </ul>
            </div>
          </div>
        </div>
